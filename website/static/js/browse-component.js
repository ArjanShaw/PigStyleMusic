// ============================================================
// browse-component.js - Shop Tile (global) + Reusable Factory
// ============================================================

// ----- GLOBAL (for the Shop tile) -----
var browseRecords = [];
var browseAllGenres = [];
var browseSelectedGenreIds = [];
var browseSelectedFormatIds = [];
var browseCurrentSearchTerm = '';
var browseNewArrivalsActive = false;   // show all records
...
function fetchBrowseGenres() {
    fetch(AppConfig.baseUrl + '/api/genres')
        ...
        .then(function(data) {
            ...
            browseAllGenres = (genres && genres.length > 0) ? genres : [];
...
function setupBrowseEventListeners() {
    ...
    // REMOVED: event listener for browseNewArrivalsBtn (button no longer exists)
    ...
    document.addEventListener('click', function(e) {
        if (!genreDropdown?.contains(e.target) && e.target !== genreBtn) genreDropdown?.classList.remove('show');
        if (!formatDropdown?.contains(e.target) && e.target !== formatBtn) formatDropdown?.classList.remove('show');
    });
...
function loadBrowseCatalogData() {
    ...
    if (browseCurrentSearchTerm) params.append('search', browseCurrentSearchTerm);
    if (browseSelectedGenreIds.length > 0) params.append('genre_ids', browseSelectedGenreIds.join(','));
    if (browseSelectedFormatIds.length > 0) params.append('format_ids', browseSelectedFormatIds.join(','));
    
    // if browseNewArrivalsActive is false, don't add created_after filter (show all)
    ...
    fetch(AppConfig.baseUrl + '/config/LAST_SEEN_CUTOFF_DATE')
        ...
        .then(function(data) {
            if (data.status === 'success') {
                ...
                if (browseNewVinylActive) records = records.filter(function(r) { return isBrowseNewVinyl(r); });
...
function updatePaginationUI() {
    ...
    if (showingSpan) showingSpan.textContent = browseTotalRecords > 0 ? start + '-' + end : '0';
    if (totalSpan) totalSpan.textContent = browseTotalRecords;
    if (pageInfo) pageInfo.textContent = browseCurrentPage + ' / ' + browseTotalPages;
...
function createBrowseRecordCard(record) {
    ...
    if (genreName) innerHtml += '<div class="browse-record-card-genre"><i class="fas fa-music"></i> ' + escapeBrowseHtml(genreName) + '</div>';
    if (formatName) innerHtml += '<div class="browse-record-card-format"><i class="fas fa-record-vinyl"></i> ' + escapeBrowseHtml(formatName) + '</div>';
    ...
    card.addEventListener('click', function() {
        ...
        else alert('Popup not available. Please refresh the page.');
    });
    return card;
}
...
// ============================================================
// ----- NEW REUSABLE FACTORY (for New Arrivals and future tiles) -----
// ============================================================
